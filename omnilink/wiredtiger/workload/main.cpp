...
struct config {
  ...
  config(
      std::initializer_list<std::pair<std::string_view, std::string>> pairs) {
    std::ostringstream out;
    bool first = true;
...
template <typename Self, typename T> struct HoldsPtr : public NoCopy {
  HoldsPtr() : ptr{nullptr} {}

  HoldsPtr(HoldsPtr<Self, T> &&other) : ptr{other.ptr} { other.ptr = nullptr; }
...
struct WtConnection : public HoldsPtr<WtConnection, WT_CONNECTION> {
  WtConnection() {
    ...
    WT_CONNECTION *conn;
    errorCheck(wiredtiger_open(tmpPath.c_str(), nullptr,
                               "create=true,log=(enabled=false)", &conn));
    setPtr(conn);
  }

  void cleanUp(WT_CONNECTION *conn) { errorCheck(conn->close(conn, nullptr)); }
};
...
struct WtSession : public HoldsPtr<WtSession, WT_SESSION> {
  WtSession(WtConnection &conn) {
    ...
    setPtr(session);
    errorCheck(session->create(session, "table:mytable",
...
struct WtCursor : public HoldsPtr<WtCursor, WT_CURSOR> {
  WtCursor(WtSession &session) {
    WT_CURSOR *cursor;
    errorCheck(session->open_cursor(session, "table:mytable", nullptr, nullptr,
                                    &cursor));
    setPtr(cursor);
  }

  void cleanUp(WT_CURSOR *cursor) { errorCheck(cursor->close(cursor)); }
};

struct WorkloadContext
    : public omnilink::WorkloadContext<WorkloadContext, Storage::AnyOperation> {
  ...
  struct RunnerDefns : public WorkloadContext::RunnerDefnsBase<RunnerDefns> {
    ...
    uint64_t find_random_kv() {
      auto read_existing = [&]() -> uint64_t {
        auto idx = std::uniform_int_distribution<std::size_t>{
            0, workload_context.existing_kvs.size() - 1}(this->rand);
      ...
      auto kv_opt = [&]() -> std::optional<uint64_t> {
        ...
        if (workload_context.existing_kvs.size() <
            workload_context.max_kv_count) {
          return {};
        } else {
      ...
      if (kv_opt.has_value()) {
        ...
        if (workload_context.existing_kvs.size() <
            workload_context.max_kv_count) {
          auto kv = std::uniform_int_distribution{
              0, std::numeric_limits<int32_t>::max()}(this->rand);
    ...
    uint64_t
    find_timestamp_at(std::initializer_list<uint64_t> lower_constraints,
                      std::initializer_list<uint64_t> upper_constraints = {
      ...
      return std::uniform_int_distribution<uint64_t>{lower_bound,
                                                     upper_bound}(this->rand);
    }
    ...
    void perform_operation(Ctx<Storage::PrepareTransaction> &ctx) {
      ...
      ctx.refine_op_start();
      int ret = session->prepare_transaction(
          session, config{
    ...
    void perform_operation(Ctx<Storage::RollbackToStable> &ctx) {
      ...
      std::unique_lock oldest_timestamp_lck{
          workload_context.oldest_timestamp_m};
      std::unique_lock stable_timestamp_lck{
          workload_context.stable_timestamp_m};
      std::unique_lock active_read_timestamps_lck{
          workload_context.active_read_timestamps_m};
      ...
      ctx.refine_op_start();
      int ret = workload_context.connection->rollback_to_stable(
          workload_context.connection, nullptr);
      ctx.refine_op_end();
      ctx.op.n = "n";
      ctx.op._did_abort = ret != 0;
      ctx.op._meta = std::map<std::string, int>{{"result_code", ret}};
    ...
    void perform_operation(Ctx<Storage::SetOldestTimestamp> &ctx) {
      ...
      std::unique_lock stable_timestamp_lck{
          workload_context.stable_timestamp_m};
      uint64_t ts = find_timestamp_at({workload_context.oldest_timestamp},
                                      {workload_context.stable_timestamp});
      // 0 value crashes WT due to assertion
      if (ts == 0) {
        ctx.unsupported();
      }
      ctx.refine_op_start();
      int ret = workload_context.connection->set_timestamp(
          workload_context.connection, config{{"oldest_timestamp", hex64(ts)}});
      ctx.refine_op_end();
      if (ret == 0) {
    ...
    void perform_operation(Ctx<Storage::SetStableTimestamp> &ctx) {
      ...
      ctx.refine_op_start();
      int ret = workload_context.connection->set_timestamp(
          workload_context.connection, config{{"stable_timestamp", hex64(ts)}});
      ctx.refine_op_end();
      if (ret == 0) {
    ...
    void perform_operation(Ctx<Storage::StartTransaction> &ctx) {
      ...
      std::shared_lock connection_lck{workload_context.connection_m};
      std::shared_lock oldest_timestamp_lck{
          workload_context.oldest_timestamp_m};
      auto read_timestamp =
          find_timestamp_at({workload_context.oldest_timestamp});
      {
        std::unique_lock active_read_timestamps_lck{
            workload_context.active_read_timestamps_m};
      ...
      ctx.refine_op_start();
      int ret = session->begin_transaction(
          session, config{{"read_timestamp", hex64(read_timestamp)}});
      ctx.refine_op_end();
      if (ret == 0) {
    ...
    void perform_operation(Ctx<Storage::TransactionWrite> &ctx) {
      ...
      auto key = find_random_kv();
      auto value =
          transactionId; // the spec assumes this, let's go with it for now
      auto keyStr = std::to_string(key);
